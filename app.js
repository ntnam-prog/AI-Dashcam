...
function centerCorridorAtY(y,w,h){
  const topY=h*0.40, bottomY=h*0.98;
  const t=Math.max(0,Math.min(1,(y-topY)/Math.max(1,bottomY-topY)));
  const halfTop=w*0.055, halfBottom=w*0.20;
  const half=halfTop+(halfBottom-halfTop)*t;
  return {left:w*0.5-half,right:w*0.5+half,topY,bottomY};
}
function isInFixedCenterCorridor(tr,w,h){
  if(tr.by<h*0.38||tr.by>h*1.02)return false;
  const c=centerCorridorAtY(tr.by,w,h);
  return tr.cx>=c.left&&tr.cx<=c.right;
}
function chooseEgoLead(h){
  const w=canvas.clientWidth||1;
  return tracks.filter(t=>!t.stale&&isInFixedCenterCorridor(t,w,h))
    .sort((a,b)=>(displayDistanceForTrack(a,h).numeric??999)-(displayDistanceForTrack(b,h).numeric??999))[0]||null;
}
...
function updateReadout(lead,h){
  ...
  if(!lead){ui.distance.textContent='--.-';ui.lead.textContent='CHƯA CÓ XE TRONG KHUNG GIỮA';ui.laneMain.textContent='L2';ui.track.textContent='TRACK: chạy ngầm';ui.readout.classList.add('state-idle');return;}
  const d=displayDistanceForTrack(lead,h);ui.distance.textContent=d.main;ui.lead.textContent='XE DẪN • L2';ui.laneMain.textContent='L2';ui.track.textContent='TRACK: chạy ngầm';
...
function labelLaneForTrack(tr,w,h){
  if(isInFixedCenterCorridor(tr,w,h))return 2;
  return tr.cx < w*0.5 ? 1 : 3;
}
function drawCenterLockLayer(w,h){
  const topY=h*0.40,bottomY=h*0.98;
  const top=centerCorridorAtY(topY,w,h),bot=centerCorridorAtY(bottomY,w,h);
  ctx.save();
  ctx.fillStyle='rgba(220,30,30,.18)';
  ctx.strokeStyle='rgba(255,255,255,.85)';
  ctx.lineWidth=2;ctx.setLineDash([8,8]);
  ctx.beginPath();ctx.moveTo(top.left,topY);ctx.lineTo(top.right,topY);ctx.lineTo(bot.right,bottomY);ctx.lineTo(bot.left,bottomY);ctx.closePath();ctx.fill();ctx.stroke();ctx.setLineDash([]);
  const cx=w/2,cy=h*0.70;
  ctx.strokeStyle='rgba(255,60,60,.95)';ctx.lineWidth=3;
  ctx.beginPath();ctx.arc(cx,cy,22,0,Math.PI*2);ctx.moveTo(cx-14,cy);ctx.lineTo(cx+14,cy);ctx.moveTo(cx,cy-14);ctx.lineTo(cx,cy+14);ctx.stroke();
  ctx.font='800 13px -apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif';
  const txt='KHÓA LÀN GIỮA',tw=ctx.measureText(txt).width;
  ctx.fillStyle='rgba(0,0,0,.72)';ctx.fillRect(cx-tw/2-8,cy+28,tw+16,24);
  ctx.fillStyle='#ff3b3b';ctx.fillText(txt,cx-tw/2,cy+45);
  ctx.restore();
}
function drawTrack(tr,h,w,lead,occupied){
  if(tr.stale)return;
  const d=displayDistanceForTrack(tr,h),lane=labelLaneForTrack(tr,w,h),isLead=lead&&tr.id===lead.id;
  const color=isLead?'#ff3434':(lane===1?'#20d66b':'#2c8cff');
  ctx.save();ctx.lineWidth=isLead?3.5:2;ctx.strokeStyle=color;ctx.strokeRect(tr.box.x,tr.box.y,tr.box.w,tr.box.h);
  const label=`${isLead?'LEAD • ':''}L${lane} • ${d.text}`;
  ctx.font=`${isLead?'800 17px':'700 13px'} -apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif`;
  const tw=ctx.measureText(label).width,pad=7,bh=isLead?30:24;
  let x=Math.max(6,Math.min(tr.box.x+tr.box.w/2-(tw+pad*2)/2,w-(tw+pad*2)-6));
  let y=Math.max(8,tr.box.y-bh-6);
  const rect=()=>({x,y,w:tw+pad*2,h:bh});
  function overlaps(a,b){return !(a.x+a.w<b.x||b.x+b.w<a.x||a.y+a.h<b.y||b.y+b.h<a.y);}
  let r=rect(),tries=0;
  while(occupied.some(o=>overlaps(r,o))&&tries<6){y=Math.max(8,y-(bh+6));r=rect();tries++;}
  if(occupied.some(o=>overlaps(r,o))){y=Math.min(h-bh-8,tr.box.y+tr.box.h+6);r=rect();}
  occupied.push(r);
  ctx.fillStyle='rgba(0,0,0,.82)';ctx.fillRect(r.x,r.y,r.w,r.h);
  ctx.strokeStyle=color;ctx.lineWidth=1.5;ctx.strokeRect(r.x,r.y,r.w,r.h);
  ctx.fillStyle=color;ctx.fillText(label,r.x+pad,r.y+(isLead?21:17));ctx.restore();
}
function render(predictions){
  ...
  if(!calibrationMode)drawCenterLockLayer(w,h);
  const occupied=[];
  if(cfg.showAll)for(const tr of tracks)drawTrack(tr,h,w,lead,occupied);else if(lead)drawTrack(lead,h,w,lead,occupied);
}
